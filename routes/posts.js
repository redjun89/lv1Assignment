...
router.get("/posts/:_postId", async (req, res) => {
  try {
    ...
    const { title, nickname, createdAt, content } = post;
    res.json({ post: { title, nickname, createdAt, content } });
  } catch (error) {
    console.error(error);
...
router.delete("/posts/:_postId", authMiddleware, async (req, res) => {
  try {
    ...
    if (post) {
      ...
      res.status(401).json({ error: "게시글 삭제에 실패하였습니다." });
    }
  } catch (error) {
    console.error(error);
    res.status(400).json({ message: '서버 에러' });
  }
});
